...
output_file <- 'svm_spatial_cluster_{type}_{format(timestamp, "%Y%m%d_%H%M")}'
...
log_text <- glue(
  '=====================================================================
SVM canopy fuels prediction from UAS metrics
...
'
  ...
  ml_rfe[[glue('{response_i}_svm_rfe_spatial_folds')]] <- ml_profile
  ...
  ml_models[[glue('{response_i}_svm_rfe_spatial_folds')]] <- ml_train 
  ml_results[[glue('{response_i}_svm_rfe_spatial_folds')]] <-
    ml_train$results %>%
    add_column(response_var = response_i,
               method = 'svm_rfe_spatial_folds',
               .before = 1)
  ...
  ml_best[[glue('{response_i}_svm_rfe_spatial_folds')]] <- ml_stats
